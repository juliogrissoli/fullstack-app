...
export async function POST(request: NextRequest) {
  ...
  const { lead_id, trigger } = body; // trigger: 'novo_lead' | 'visita' | 'proposta' | 'fechamento'
  ...
  await supabase.from('agent_logs').insert({
    ...
    decision: nivel,
    score_impact: score,
    message: `Score ${score}: ${nivel}`
  });
  agentLog.push({ agente: 'Árion', decisao: nivel, score });

  // 3. Themis — Validação Jurídica
  if (trigger === 'visita' || trigger === 'proposta' || trigger === 'fechamento') {
    ...
    await supabase.from('agent_logs').insert({
      ...
      lead_id,
      action: 'validar_nexo',
      decision: nexoOk ? 'seguro' : 'risco',
      message: nexoOk ? 'Nexo Causal confirmado' : 'ALERTA: Visita sem registro'
    });
    agentLog.push({ agente: 'Themis', decisao: nexoOk ? 'seguro' : 'risco' });
  }

  // 4. Plutus — Cálculo Financeiro
  if (trigger === 'proposta' || trigger === 'fechamento') {
    ...
    await supabase.from('agent_logs').insert({
      agent_name: 'plutus',
      lead_id,
      action: 'calcular_split',
      decision: 'viavel',
      message: `Comissão: R$ ${comissao.toLocaleString('pt-BR')} | Corretor: R$ ${splitCorretor.toLocaleString('pt-BR')}`
    });
    agentLog.push({ agente: 'Plutus', decisao: 'viavel', comissao, splitCorretor });
  }

  // 5. Yara — Decisão Final
  if (trigger === 'novo_lead') {
    if (nivel === 'ATACAR') {
      decisao.acao = 'notificar_corretor';
      decisao.mensagem = '🔥 Lead QUENTE! Contate imediatamente.';
      decisao.proximoPasso = 'Enviar mensagem via Resend e WhatsApp';
    } else if (nivel === 'NUTRIR') {
      decisao.acao = 'nutrir';
      decisao.mensagem = '⚡ Lead MORNO. Iniciar sequência de nutrição.';
      decisao.proximoPasso = 'Enviar White Paper e agendar follow-up em 24h';
    } else {
      decisao.acao = 'arquivar';
      decisao.mensagem = '❄️ Lead FRIO. Arquivar sem queimar tempo.';
      decisao.proximoPasso = 'Reavaliar em 30 dias';
    }
  }
