...
export default function HomeWidgets() {
  ...
  const handleBuyAbility = async () => {
    ...
    const data = await res.json();
    setBuyResult(data);
    if (data.score >= 70) router.push('/qualifica');
    setBuyLoading(false);
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto px-4 py-12">
      <div className="bg-gray-800 rounded-xl p-8 border border-gray-700 hover:border-[#D4AF37] transition">
        <h3 className="text-2xl font-bold text-[#D4AF37] mb-2">🏠 Quanto vale seu imóvel?</h3>
        <p className="text-gray-400 mb-6">Descubra em 10 segundos com nossa IA de precificação.</p>
        <input type="text" placeholder="Digite o endereço completo" value={avmEndereco}
        ...
        <button onClick={handleAVM} disabled={avmLoading}
        ...
        {avmResultado && (
          <div className="mt-4 p-4 bg-gray-700 rounded-lg">
            ...
            <button onClick={() => router.push('/qualifica')}
              className="mt-3 text-[#D4AF37] hover:underline text-sm">Quero vender com proteção Anjoimob →</button>
          </div>
        )}
      </div>

      <div className="bg-gray-800 rounded-xl p-8 border border-gray-700 hover:border-[#D4AF37] transition">
        <h3 className="text-2xl font-bold text-[#D4AF37] mb-2">💰 Quanto você pode investir?</h3>
        <p className="text-gray-400 mb-6">Responda 3 perguntas e descubra seu poder de compra.</p>
        <input type="number" placeholder="Renda mensal (R$)" value={buyAbility.renda || ''}
        ...
        <input type="number" placeholder="Valor de entrada (R$)" value={buyAbility.entrada || ''}
          onChange={(e) => setBuyAbility({ ...buyAbility, entrada: Number(e.target.value) })}
          className="w-full p-3 bg-gray-700 border border-gray-600 rounded-lg mb-3 text-white focus:outline-none focus:border-[#D4AF37]" />
        <select value={buyAbility.prazo}
        ...
        <button onClick={handleBuyAbility} disabled={buyLoading}
          className="btn-gold-glow w-full py-3 rounded-lg font-semibold disabled:opacity-50">
