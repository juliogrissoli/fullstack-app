...
export async function analisarDocumento(
  ...
  const regra = regrasTipo[tipoDocumento];
  if (regra) {
    alertas.push(...regra.alertas);
    scoreRisco += regra.score;
  }
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="background:#1a1a2e;color:#fff;padding:24px;border-radius:12px;border:1px solid #D4AF37;">
          ...
          <p style="color:#9ca3af;font-size:12px;margin-top:24px;">Alerta automático do Agente Themis.</p>
        </div>`,
    });
    return true;
  } catch {
